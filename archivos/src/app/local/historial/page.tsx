...
type V2Reservation = {
  ...
  kitchenTickets?: Array<
    Omit<V2KitchenTicket, "items"> & { items: V2ReservationOrderLineItem[] }
  >;
};

type KitchenHistoryEntry = {
  id: string;
  date: string;
  time: string;
  source: "reservation" | "delivery";
  sourceLabel: string;
  client: string;
...
function getKitchenStatusLabel(entry: KitchenHistoryEntry) {
  if (entry.status === "pending") return "Pendiente";
  if (entry.status === "preparing") return "En preparación";
  if (entry.status === "ready") return "Lista";
  return entry.source === "reservation" ? "Servida" : "Despachada";
}

function getKitchenItemsSummary(entry: KitchenHistoryEntry) {
  return entry.items.map((item) => `${item.quantity}× ${item.name}`).join(" · ");
}

function subtractKitchenTicketItems(
  items: Array<{ name: string; quantity: number }>,
  tickets: Array<{ items: Array<{ name: string; quantity: number }> }>,
) {
  ...
  return items.flatMap((item) => {
    const quantity = Number(item.quantity) - (ticketQuantities.get(item.name.trim().toLowerCase()) ?? 0);
...
export default function HistorialPage() {
  ...
  const kitchenHistory = useMemo<KitchenHistoryEntry[]>(() => {
    const reservationEntries = reservations.flatMap<KitchenHistoryEntry>((reservation) => {
      ...
      const baseItems = subtractKitchenTicketItems(reservation.orderLineItems ?? [], tickets);
      const baseEntry =
        reservation.kitchenStatus && baseItems.length > 0
          ? [{
              id: `reservation-${reservation.id}`,
              date: getKitchenEntryDate(
                reservation.kitchenCompletedAt ?? reservation.kitchenReadyAt ?? reservation.kitchenStartedAt,
                reservation.date,
              ),
              ...
              items: baseItems,
              status: reservation.kitchenStatus,
              isAddition: false,
      ...
      const ticketEntries = tickets.map((ticket) => ({
        ...
        date: getKitchenEntryDate(ticket.completedAt ?? ticket.readyAt ?? ticket.startedAt ?? ticket.createdAt, reservation.date),
        time: reservation.time,
        source: "reservation" as const,
        sourceLabel: reservation.tableName || "Mesa sin asignar",
        client: reservation.client,
      ...
      return [...baseEntry, ...ticketEntries];
    });

    const deliveryEntries = deliveries.flatMap<KitchenHistoryEntry>((delivery) => {
      const tickets = delivery.kitchenTickets ?? [];
      const baseItems = subtractKitchenTicketItems(delivery.orderItems ?? [], tickets);
      const fallbackDate = delivery.date ?? getTodayDateKey();
      const baseEntry =
        delivery.kitchenStatus && baseItems.length > 0
          ? [{
              id: `delivery-${delivery.id}`,
              date: getKitchenEntryDate(
                delivery.kitchenCompletedAt ?? delivery.kitchenReadyAt ?? delivery.kitchenStartedAt,
                fallbackDate,
              ),
              time: delivery.time,
      ...
      const ticketEntries = tickets.map((ticket) => ({
        ...
        date: getKitchenEntryDate(ticket.completedAt ?? ticket.readyAt ?? ticket.startedAt ?? ticket.createdAt, fallbackDate),
        time: delivery.time,
        source: "delivery" as const,
        sourceLabel: delivery.deliveryType === "delivery" ? "Delivery" : "Retiro",
        client: delivery.client,
    ...
    return [...reservationEntries, ...deliveryEntries];
  }, [deliveries, reservations]);

  const filteredKitchenHistory = useMemo(() => {
    const normalizedQuery = normalizeSearch(query);

    return kitchenHistory
      .filter((entry) => {
        const matchesStatus = statusFilter === "all" || entry.status === statusFilter;
        const matchesRange = isInsideRange(entry.date, rangeFilter, selectedDate, customStartDate, customEndDate);
        const matchesSearch =
          !normalizedQuery ||
          [entry.sourceLabel, entry.client, getKitchenItemsSummary(entry)]
            .join(" ")
            .toLowerCase()
            .includes(normalizedQuery);
      ...
      .sort(
        (a, b) =>
          new Date(b.completedAt ?? b.readyAt ?? b.startedAt ?? b.enteredAt ?? `${b.date}T${b.time}:00`).getTime() -
          new Date(a.completedAt ?? a.readyAt ?? a.startedAt ?? a.enteredAt ?? `${a.date}T${a.time}:00`).getTime(),
      );
  }, [customEndDate, customStartDate, kitchenHistory, query, rangeFilter, selectedDate, statusFilter]);
      ...
      <div className="flex h-full min-h-0 flex-col">
          ...
          actions={
            <div className="flex items-center gap-2">
              ...
              <div className="flex rounded-xl border border-slate-200 bg-white p-1">
                ...
                <button
                  type="button"
                  onClick={() => selectHistoryTab("cocina")}
                  className={`rounded-lg px-4 py-2 text-sm font-semibold transition ${
                    activeTab === "cocina"
                      ? "bg-emerald-600 text-white"
                      : "text-slate-600 hover:bg-slate-50 hover:text-slate-950"
        ...
        <div className="historial-top-metrics mt-4 grid shrink-0 gap-3 md:grid-cols-3 xl:grid-cols-6">
          <V2MetricCard
            ...
            tone="blue"
            icon={activeTab === "envios" ? <PackageCheck size={20} /> : activeTab === "reservas" ? <CalendarDays size={20} /> : <ChefHat size={20} />}
          />
          <V2MetricCard
            label={activeTab === "cocina" ? "Pendientes" : "Activos"}
            value={activeTab === "cocina" ? filteredKitchenHistory.filter((item) => item.status === "pending").length : activeOperationalCount}
            helper={activeTab === "cocina" ? "Esperando inicio" : activeTab === "envios" ? "Confirmados" : "Confirmadas"}
            tone="green"
            icon={<CheckCircle2 size={20} />}
          />
          <V2MetricCard
            label={activeTab === "cocina" ? "En preparación" : "Cerrados"}
            value={activeTab === "cocina" ? filteredKitchenHistory.filter((item) => item.status === "preparing").length : closedCount}
            helper={activeTab === "cocina" ? "Trabajando ahora" : "Completados/cancelados"}
            tone="slate"
            icon={<History size={20} />}
          />
          <V2MetricCard
            label={activeTab === "envios" ? "Facturado" : activeTab === "reservas" ? "Personas" : "Listas"}
            value={activeTab === "envios" ? formatMoney(moneyTotal) : activeTab === "reservas" ? peopleTotal : filteredKitchenHistory.filter((item) => item.status === "ready").length}
          ...
          <V2MetricCard
            label={activeTab === "cocina" ? "Servidas" : "Efectivo"}
            value={activeTab === "cocina" ? filteredKitchenHistory.filter((item) => item.status === "completed" && item.source === "reservation").length : formatMoney(activeTab === "envios" ? deliveryCashTotal : reservationCashTotal)}
            helper={
              activeTab === "cocina"
                ? "Comandas de mesa"
                : activeTab === "envios"
          ...
          <V2MetricCard
            label={activeTab === "cocina" ? "Despachadas" : "Tarjeta / no efectivo"}
            value={activeTab === "cocina" ? filteredKitchenHistory.filter((item) => item.status === "completed" && item.source === "delivery").length : formatMoney(activeTab === "envios" ? deliveryNonCashTotal : reservationNonCashTotal)}
            helper={
              activeTab === "cocina"
                ? "Delivery y retiro"
                : activeTab === "envios"
            ...
            <V2Select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}>
              ...
              {activeTab === "envios" ? (
                ...
                <>
                  ...
                  <option value="completed">Servidas / despachadas</option>
                </>
              )}
            </V2Select>
        ...
        <div className="v2-history-scrollbar mt-4 min-h-0 flex-1 overflow-y-auto pr-1">
          {activeTab === "envios" ? (
            ...
            <div className="space-y-2">
              {filteredKitchenHistory.map((entry) => {
                const isOpen = openId === entry.id;
                const timeline = [
                  { label: "Ingresó a cocina", value: entry.enteredAt },
                  { label: "Comenzó preparación", value: entry.startedAt },
                  { label: "Marcada lista", value: entry.readyAt },
                  {
                    label: entry.source === "reservation" ? "Servida" : "Despachada",
                  ...
                  <V2Card key={entry.id} className="overflow-hidden p-0 shadow-sm">
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 truncate font-semibold text-slate-950">{entry.client}</p>
                      </div>
                      <div className="min-w-0">
                        <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                          Comanda
                        ...
                        <p className="mt-1 truncate text-sm text-slate-500">
                          {entry.isAddition ? "Agregado · " : ""}
                          {getKitchenItemsSummary(entry)}
                        </p>
                      </div>
                      <div>
                        <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                    ...
                    {isOpen ? (
                      <div className="border-t border-slate-100 bg-slate-50/60 px-7 py-5">
                        <div className="ml-4 space-y-4 border-l border-slate-200 pl-6">
                          {timeline.map((item, index) => (
                            <div key={`${entry.id}-${item.label}`} className="grid grid-cols-[110px_24px_1fr] gap-3">
                              <div className="pt-0.5 text-right text-xs font-semibold text-slate-500">
                                {item.value ? new Date(item.value).toLocaleTimeString("es-AR") : "—"}
                              </div>
                              <div className="relative flex justify-center">
              ...
              {filteredKitchenHistory.length === 0 ? (
                <V2Card>
                  <div className="flex h-[260px] items-center justify-center text-center">
                    <div>
                      <ChefHat className="mx-auto text-slate-300" size={42} />
                      <p className="mt-3 font-semibold text-slate-950">No hay movimientos de cocina</p>
