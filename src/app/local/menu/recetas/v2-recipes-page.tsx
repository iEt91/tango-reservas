...
import { saveBusinessMenuItemAction } from "../actions";
import { saveBusinessMenuRecipeAction } from "./actions";
import type {
  BusinessMenuCategoryEditor,
  BusinessMenuItemEditor,
} from "@/lib/menu/business-menu-contract";
import type { BusinessRecipe } from "@/lib/recipes/business-recipe-contract";
import type { BusinessStockProductSnapshot } from "@/lib/stock/business-stock-contract";
...
type V2RecipesPersistence =
  | "local"
  | "supabase";

type V2RecipesPageProps = {
  initialRecipes?: BusinessRecipe[];
  initialMenuItems?: BusinessMenuItemEditor[];
  initialMenuCategories?: BusinessMenuCategoryEditor[];
  initialStockProducts?: BusinessStockProductSnapshot[];
  recipePersistence?: V2RecipesPersistence;
...
function syncRecipesWithMenu(
  ...
  return menuItems.map((item) => {
    ...
    return {
      ...
      persisted:
        storedRecipe
          ? storedRecipe.persisted ?? true
          : false,
    };
  });
}

function mapPersistentMenuItem(
...
function mapPersistentMenuCategory(
  category: BusinessMenuCategoryEditor,
): V2RecipeMenuCategory {
  return {
    id: category.id,
    name: category.name,
...
function mapPersistentRecipe(
  recipe: BusinessRecipe,
  stockProducts: V2RecipeStockProduct[],
): V2RecipeConfig {
...
function buildPersistentInitialState(
  initialMenuItems: BusinessMenuItemEditor[],
  initialMenuCategories: BusinessMenuCategoryEditor[],
  initialRecipes: BusinessRecipe[],
  initialStockProducts: BusinessStockProductSnapshot[],
  ...
  const menuItems =
    initialMenuItems.map(
      mapPersistentMenuItem,
    );
  const menuCategories =
    initialMenuCategories.map(
      mapPersistentMenuCategory,
    );
  const stockProducts =
    initialStockProducts.map(
...
export function V2RecipesPage({
  initialRecipes = [],
  initialMenuItems = [],
  initialMenuCategories = [],
  initialStockProducts = [],
  recipePersistence = "local",
  canManageRecipes = true,
  canCreateMenuItems = true,
  canViewStock = true,
}: V2RecipesPageProps = {}) {
  const usesSupabaseRecipes =
    recipePersistence === "supabase";
  const persistentInitialState =
    buildPersistentInitialState(
      initialMenuItems,
      initialMenuCategories,
      initialRecipes,
      initialStockProducts,
  ...
  const [recipes, setRecipes] =
    useState<V2RecipeConfig[]>(
      () =>
        usesSupabaseRecipes
          ? persistentInitialState.recipes
          : [],
    );
  const [
    activeRecipeId,
    ...
    () =>
      usesSupabaseRecipes
        ? persistentInitialState.recipes[0]?.id
          ?? ""
        : "",
  );
  const [menuItems, setMenuItems] =
    useState<V2RecipeMenuItem[]>(
      () =>
        usesSupabaseRecipes
          ? persistentInitialState.menuItems
          : v2MenuItems,
    );
  const [
    menuCategories,
    setMenuCategories,
  ] = useState<V2RecipeMenuCategory[]>(
    () =>
      usesSupabaseRecipes
        ? persistentInitialState.menuCategories
        : v2MenuCategories,
  );
  const [
    stockProducts,
    setStockProducts,
  ] = useState<V2RecipeStockProduct[]>(
    () =>
      usesSupabaseRecipes
        ? persistentInitialState.stockProducts
        : v2StockProducts,
  );
  const [
    expandedCategoryIds,
  ...
  const [
    isNewPlatePopupOpen,
    setIsNewPlatePopupOpen,
  ] = useState(false);
  const [
    newPlateForm,
    setNewPlateForm,
  ] = useState<V2NewRecipePlateForm>(
    () =>
      createEmptyNewRecipePlateForm(),
  );

  const loadMenuStockAndRecipes = useCallback(
    (
      options?: {
        preserveActive?: boolean;
      },
    ) => {
    if (usesSupabaseRecipes) return;

    const nextMenuItems =
      readLocalStorageList<V2RecipeMenuItem>(
        MENU_ITEMS_STORAGE_KEY,
        v2MenuItems,
      );
    const nextMenuCategories =
      readLocalStorageList<V2RecipeMenuCategory>(
        MENU_CATEGORIES_STORAGE_KEY,
        v2MenuCategories,
      );
    const nextStockProducts =
      readLocalStorageList<V2RecipeStockProduct>(
  ...
  async function persistRecipes(
    ...
    if (!usesSupabaseRecipes) {
      writeRecipesToConfig(
        nextRecipes,
      );
      setSaveStatus("saved");
      return;
    ...
    if (!canManageRecipes) {
      setOperationError(
        "Tu rol tiene acceso de solo lectura a Recetas.",
      );
      return;
    }
    ...
    try {
      const result =
        await saveBusinessMenuRecipeAction({
          menuItemId:
            activeRecipe.menuItemId,
          recipe: {
            name:
              activeRecipe.name,
            preparationTimeSeconds:
      ...
      setOperationError(
        "No se pudo guardar la receta. Revisá la conexión y reintentá.",
      );
      setSaveStatus("idle");
    }
  }
  ...
  function updateActiveRecipeName(name: string) {
    if (!activeRecipe || !canManageRecipes) return;
  ...
  function openNewPlatePopup() {
    if (
      ...
      setOperationError(
        "Solo el dueño o un administrador pueden crear platos desde Recetas.",
      );
      return;
    }
  ...
  async function createMenuItemFromRecipes() {
    const cleanName =
      newPlateForm.name.trim()
      || "Nuevo plato sin nombre";
    const preparationTimeSeconds =
      parsePreparationTime(
        newPlateForm.preparationTime,
      )
      ?? DEFAULT_PREPARATION_TIME_SECONDS;
    ...
    if (usesSupabaseRecipes) {
      ...
      if (!canManageRecipes) {
        setOperationError(
          "Tu rol no puede guardar recetas.",
        );
        return;
      }
      ...
      try {
        const menuResult =
          await saveBusinessMenuItemAction({
            menuItemId: null,
            item: {
              categoryId:
                newPlateForm.categoryId
                || null,
              name: cleanName,
              description:
                newPlateForm.description.trim(),
              price:
                Number(
                  newPlateForm.price,
                ) || 0,
        ...
        const draftRecipe:
          V2RecipeConfig = {
            ...
            preparationTimeSeconds,
            ingredients:
              newPlateForm.ingredients,
            persisted: false,
          };

        let savedRecipe:
          V2RecipeConfig | null = null;
        let recipeError = "";
        ...
        try {
            ...
            await saveBusinessMenuRecipeAction({
              ...
              recipe: {
                name:
                  draftRecipe.name,
                preparationTimeSeconds:
                  draftRecipe.preparationTimeSeconds,
                ingredients:
                  draftRecipe.ingredients.map(
                    (ingredient) => ({
                      stockProductId:
                        ingredient.stockProductId,
                      quantity:
          ...
          recipeError =
            "No se pudo completar el guardado de la receta.";
        }
        ...
        if (recipeError) {
          setOperationError(
            `El plato se creó, pero la receta quedó pendiente: ${recipeError}`,
          );
          setSaveStatus("idle");
        } else {
          setSaveStatus("saved");
        ...
        setOperationError(
          "No se pudo crear el plato. Revisá la conexión y reintentá.",
        );
        return;
      } finally {
  ...
  return (
    <V2AppShell>
      <div className="flex h-full min-h-0 flex-col">
        ...
        {usesSupabaseRecipes && !canViewStock ? (
          <div className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
            Tu rol puede ver Recetas, pero no tiene lectura de Stock. Los nombres y saldos de insumos no están disponibles y los cambios de ingredientes quedan bloqueados.
          </div>
        ) : null}
                ...
                <div className="flex h-full min-h-0 flex-col">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button
                      ...
                      className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-40"
                  ...
                  <div className="mt-6 space-y-4">
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm leading-6 text-emerald-900">
                            Esta receta define qué insumos deberá descontar Stock cuando se
                            conecte el consumo automático de ventas. E30B no descuenta Stock automáticamente.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      ...
                      <p className="mt-2 text-sm font-semibold text-slate-800">
                        {usesSupabaseRecipes
                          && activeRecipe.persisted === false
                          ? "Pendiente de guardar en Supabase"
                            ...
                            >
                              <div className="grid gap-3 lg:grid-cols-[1fr_120px_120px_44px]">
                                ...
                                <button
                                  ...
                                  className="mt-6 inline-flex h-11 w-11 items-center justify-center rounded-xl border border-red-200 bg-red-50 text-red-600 transition hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-40"
          ...
          <div className="flex max-h-[88vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
            <div className="flex items-start justify-between gap-4 border-b border-slate-200 p-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Creá el plato del menú y guardá su receta de stock en el mismo flujo.
                </p>
              </div>
            ...
            <div className="flex justify-end gap-3 border-t border-slate-200 p-5">
              ...
              <V2Button
                ...
                variant="primary"
                icon={<Save size={16} />}
                onClick={() => void createMenuItemFromRecipes()}
                disabled={isCreatingPlate}
              >
                {isCreatingPlate
                  ? "Guardando..."
                  : "Guardar plato"}
