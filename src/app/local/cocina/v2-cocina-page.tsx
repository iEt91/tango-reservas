...
function mapPersistentCommand(
  command: BusinessKitchenCommand,
): KitchenCommand {
  return {
    id:
      command.id,
...
export function V2CocinaPage({
  ...
  const refreshPersistentSnapshot =
    useCallback(
      async () => {
        if (
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      void refreshPersistentSnapshot();
    };
                ...
                >(
                  (ticket) => {
                    ...
                    return {
                      ...sharedCommand,
                      id:
                        `reservation-${reservation.id}-${ticket.id}`,
                      ticketId:
                        ticket.id,
                      isAddition:
                        true,
                ...
                >(
                  (ticket) => {
                    ...
                    return {
                      ...sharedCommand,
                      id:
                        `delivery-${delivery.id}-${ticket.id}`,
                      ticketId:
                        ticket.id,
                      isAddition:
                        true,
      ...
      return [
        ...
        (
          ...
          const firstTime =
            new Date(
              first.enteredAt
              ?? `${today}T${first.time}:00`,
            ).getTime();
          const secondTime =
            new Date(
              second.enteredAt
              ?? `${today}T${second.time}:00`,
            ).getTime();
  ...
  async function updateCommand(
    ...
    if (
      ...
      if (
        ...
        setOperationError(
          "No tenés permisos para modificar Cocina.",
        );
        return;
      }
      ...
      if (!command.orderId) {
        setOperationError(
          "La comanda persistente no tiene un pedido válido.",
        );
        return;
      }
    ...
    <V2AppShell>
      <div className="flex h-full min-h-0 flex-col">
        ...
        && (
          ...
          <div
            className={cn(
              "mt-3 shrink-0 rounded-xl border px-3 py-2 text-sm",
              operationError
