...
assert.doesNotMatch(envTemplate, /sb_secret_[A-Za-z0-9_-]{20,}/u);
...
assert.match(context, /stagingProjectRef === productionProjectRef/);
...
assert.match(preflightSql, /profiles_with_unknown_auth_user/);
...
assert.match(postflightSql, /FORCE RLS|relforcerowsecurity/i);
assert.match(postflightSql, /private\.has_business_role/);
assert.match(postflightSql, /anon conserva SELECT/);
console.log("✓ el postflight falla ante configuraciones inseguras");
...
console.log("✓ comandos de staging integrados sin acceso remoto en QA");
...
assert.match(stdout, /Integridad de migraciones aprobada/);
...
assert.match(rollout, /Nunca aplicar primero en producción/);
assert.match(rollout, /dos proyectos Supabase diferentes/);
assert.match(rollout, /prueba real de aislamiento/);
console.log("✓ el runbook exige separación y pruebas negativas");
...
console.log("✓ la prueba de aislamiento queda definida como P0");
...
console.log("Todos los casos de preparación de staging pasaron (10).");
